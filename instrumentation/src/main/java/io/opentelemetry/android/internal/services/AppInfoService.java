...
public class AppInfoService implements Service {
    ...
    private long getAvailableSpace(File directory, long maxSpaceNeeded) {
        logger.log(Level.FINER, String.format("Getting available space for %s, max needed is: %s", directory, maxSpaceNeeded));
        try {
    ...
    private long getLegacyAvailableSpace(File directory, long maxSpaceNeeded) {
        logger.log(Level.FINER, String.format("Getting legacy available space for %s max needed is: %s", directory, maxSpaceNeeded));
